...
EFI_STATUS pe_kernel_get_headers_size(const void *base, size_t *ret_headers_size) {
        ...
        *ret_headers_size = pe->OptionalHeader.SizeOfHeaders;
        return EFI_SUCCESS;
}
...
EFI_STATUS pe_kernel_get_image_base(const void *base, uint64_t *ret_image_base) {
        assert(base);
        assert(ret_image_base);
...
/* Apply PE base relocations to a loaded image.
 *
 * Some kernel PE images (notably Ubuntu arm64 zboot-wrapped kernels) contain base
 * relocation entries because the linker emits absolute address references. When we
 * load the kernel at a different address than ImageBase, these references must be
 * fixed up or the kernel will crash accessing memory at ImageBase offsets.
 *
 * See: https://bugs.launchpad.net/ubuntu/+source/linux/+bug/2098111
...
EFI_STATUS pe_kernel_apply_relocations(
        ...
        int64_t delta = (int64_t) actual_base - (int64_t) image_base;
        if (delta == 0)
                return EFI_SUCCESS;

        if (reloc_rva >= loaded_size || reloc_rva + reloc_size > loaded_size)
                return log_error_status(EFI_LOAD_ERROR, "Base relocation table extends outside loaded image");

        log_info("Applying PE base relocations (delta=%lld)", (long long) delta);
        ...
        while (reloc_ptr + 8 <= reloc_end) {
                uint32_t page_rva = *(const uint32_t *) reloc_ptr;
                uint32_t block_size = *(const uint32_t *) (reloc_ptr + 4);

                if (block_size < 8 || reloc_ptr + block_size > reloc_end)
                ...
                for (size_t i = 0; i < n_entries; i++) {
                        ...
                        case IMAGE_REL_BASED_DIR64:
                                if (fixup_rva + sizeof(uint64_t) > loaded_size)
                                        return log_error_status(EFI_LOAD_ERROR, "Relocation fixup outside loaded image");
                                *(uint64_t *) (loaded_base + fixup_rva) += delta;
                                break;
                        default:
